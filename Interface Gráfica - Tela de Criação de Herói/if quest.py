...
class SemManaError(Exception):
    def __init__(self, mana):
        super().__init__(f"Mana insuficiente: {mana} (esperado pelo menos 10)")


class ForcaInsuficienteError(Exception):
    def __init__(self, forca):
        super().__init__(f"Força insuficiente: {forca} (esperado pelo menos 10)")
...
class Personagem(ABC):
    ...
    def set_vida(self, vida):
        if 0 <= vida <= 200:
            ...
            raise ValueError(f"Vida inválida: {vida} (esperado um valor entre 0 e 200)")
    ...
    def set_nivel(self, nivel):
        if nivel >= 1:
            ...
            raise ValueError(f"Nível inválido: {nivel} (esperado um valor maior ou igual a 1)")
...
class Item:
    ...
    def set_bonus(self, bonus):
        if bonus >= 0:
            ...
            raise ValueError(f"Bônus inválido: {bonus} (esperado um valor maior ou igual a 0)")
...
class Mago(Personagem):
    ...
    def set_mana(self, mana):
        if mana < 0:
            raise ValueError(f"Mana inválida: {mana} (esperado um valor maior ou igual a 0)")
        self._mana = mana
...
class Guerreiro(Personagem):
    ...
    def set_defesa(self, defesa):
        if defesa < 0:
            raise ValueError(f"Defesa inválida: {defesa} (esperado um valor maior ou igual a 0)")
        self._defesa = defesa
    ...
    def set_forca(self, forca):
        if forca < 0:
            raise ValueError(f"Força inválida: {forca} (esperado um valor maior ou igual a 0)")
        self._forca = forca
...
class TelaCriacaoHeroi(tk.Tk):
    ...
    def __init__(self):
        ...
        titulo = tk.Label(self, text="Criação de Herói")
        titulo.pack(side="top")

        painel = tk.Frame(self)

        rotulo_nome = tk.Label(painel, text="Nome:")
        self.campo_nome = tk.Entry(painel, width=20)

        rotulo_nome.grid(row=0, column=0, padx=5, pady=5)
        self.campo_nome.grid(row=0, column=1, padx=5, pady=5)

        rotulo_vida = tk.Label(painel, text="Vida:")
        self.campo_vida = tk.Entry(painel, width=20)

        rotulo_vida.grid(row=1, column=0, padx=5, pady=5)
        self.campo_vida.grid(row=1, column=1, padx=5, pady=5)

        rotulo_classe = tk.Label(painel, text="Classe:")
        self.campo_classe = tk.Entry(painel, width=20)

        rotulo_classe.grid(row=2, column=0, padx=5, pady=5)
        self.campo_classe.grid(row=2, column=1, padx=5, pady=5)

        painel.pack(expand=True)

        self.botao_criar = tk.Button(self, text="Criar Herói")
